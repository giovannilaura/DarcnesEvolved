...
tms9918 tms9918_create(void)
{
    ...
    retval = calloc(1, sizeof(struct tms9918));
    if (retval) {
	retval->memory = calloc(1, TMS_RAMSIZE);
	if (retval->memory) {
	    retval->scanline = 0;
            video_setsize(256*DEFAULT_SCALER_COLECO, 192*DEFAULT_SCALER_COLECO);
