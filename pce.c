...
#define PSG_BUFSIZE 128
int psgBufPos=0;
u8 psgBuf [4] [PSG_BUFSIZE];



#define PCE_RAMSIZE 0x8000
#define PCE_RAMSIZE 0x20000 //joefix 128 kb ram
...
void pce_write_mem(cal_cpu cpu, unsigned long addr, unsigned char data)
{
	...
	case 0x0800: /* PSG */
                 if ( (addr & 15) == 6) {
                 printf("SHOULD OUTPUT SOUND %d\n",psgBufPos);
                 if (psgBufPos < 128) {
                    psgBuf [0] [psgBufPos++] = data;
                 }
                 else if (psgBufPos < 256) {
                    psgBuf [1] [psgBufPos++-128] = data;
                 }
                 else if (psgBufPos < 384)  {
                    psgBuf[2] [psgBufPos++-256] = data;
                 }
                 
                 else if (psgBufPos < 512) {
                    psgBuf[3] [psgBufPos++-384] = data;
                 }
                 
                 
                
                } /*
                if (psgBufPos>=PSG_BUFSIZE) {
                  psgBufPos=0;
                  snd_output_4_waves(PSG_BUFSIZE*4,psgBuf[0],psgBuf[1],psgBuf[2],psgBuf[3]);
            }
            */
	    break;
	case 0x0c00: /* Timers */
