...
typedef struct translation_buffer {
        unsigned long pixel;
} translation_buffer;

struct translation_buffer  *translation_buffer_24;
...
void video_setpal( int colors, int* red, int* green, int* blue )
{
  ...
  case 24:
    translation_buffer_24 = (translation_buffer* ) malloc( sizeof(translation_buffer)*colors );
...
//color.blue=0;
      ...
      fprintf(stderr,"colrs=%d color r=%d,g=%d,b=%d",colors,red[i],green[i],blue[i]);
      
      
      if( gdk_colormap_alloc_color( color_map, &color, TRUE, TRUE ) )
      {
        
	//translation_buffer_24[i].pixel=(red[i]*65536 + green[i]*256 + blue[i]);
        translation_buffer_24[i].pixel=color.pixel;
  ...
  case 2:
   ...
   case 4:
    ...
    {             
        ...
        buffer_16[i*2] = translation_buffer_24[ (int) cur_video_buffer[i]].pixel;
        buffer_16[i*2+1] = translation_buffer_24[ (int) cur_video_buffer[i+1]].pixel;
