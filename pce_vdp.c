...
void pce_vdp_render_sprites(pce_vdp vdp, unsigned char *cur_vbp, int behind)
{
	...
	default:
        ...
        if (sprite_vsize== 32 && cur_sprite.xpos < 48) {
            //cur_sprite.xpos-=48;
        
        }
        if (sprite_vsize== 16 && cur_sprite.xpos < 48) {
            //cur_sprite.xpos-=48;
        } 
...
int pce_vdp_periodic(pce_vdp vdp)
{
    /*if ((vdp->regs_l[11] & 0x7f) != vdp->last_width) { 
         ...
         vdp->display_width = (vdp->last_width*2 + 1) << 3;
         vdp->last_height=480;
         video_setsize((vdp->display_width*2), (vdp->last_height*2));
         
         printf("Set Size %d %d\n",vdp->display_width,vdp->last_height);
...
/*  	deb_printf("pce_vdp: RCR! (%d)\n", vdp->cur_scanline); */
	...
	if ((vdp->regs_l[11] & 0x7f) != vdp->last_width) {
            vdp->last_width = vdp->regs_l[11] & 0x7f;
	    vdp->display_width = (vdp->last_width*2 + 1) << 3;
            //joe tell to video_x the resolution
            printf("Changing width %d\n",vdp->display_width);
            pce_display_width=vdp->display_width;
            /*
            addams family
            if (pce_display_width==696) {
                   
                   vdp->last_height=448;
            }
            //shadowgate
            if (pce_display_width==680) {
                    vdp->last_height=448;
                   
            }
            //Browning
            if (pce_display_width==504) {
                    vdp->last_height=320;
                   
            }
            
            //afterburner
            if (pce_display_width==488) {
                   vdp->last_height=800;
            }
            //adventure island
            if (pce_display_width==256) {
                   vdp->last_height=478;
                   
                 
            }
            //dragoon slayer
            if (pce_display_width==664) {
                     vdp->last_height=600;
                     
                  
            }
            if (pce_display_width==600) {
                     vdp->last_height=600;
                     
                     
            }
            //Dungeon Master Therons Quest
            if (pce_display_width==632) {
                     vdp->last_height=512;
                             
            }

            //EXILE
            if (pce_display_width==344) {
                     vdp->last_height=512;
            ...
            //Genocide
            if (pce_display_width==280) {
                     vdp->last_height=512;
                     
            }
            
            //It came from the desert
            if (pce_display_width==536) {
                     vdp->last_height=512;
                     
            }

            //MOST GAMES
            if (pce_display_width==512) {
                     vdp->last_height=480;
            ...
            */
            ...
            if (pce_display_width==372) {
                vdp->last_height=480;
            }

            if (pce_display_width==640) {
                vdp->last_height=512;
            }

            if (pce_display_width==564) {
                vdp->last_height=484;
            }

            if (pce_display_width==377*2) {
                vdp->last_height=484;
            }

            if (pce_display_width==565*2) {
                vdp->last_height=484;
            }
            if (pce_display_width==696) {
                vdp->last_height=800;
           ...
           if (pce_display_width==632) {
            ...
            //ShapeShifter,Shadowgate
            if (getenv("PCEFORCEH") != NULL) {
                    int hei=atoi(getenv("PCEFORCEH"))*2;
                    vdp->last_height=hei; 
                    printf ("FORCING HEIGHT TO %d\n",hei);
            }
            ...
            if (pce_display_width == 1016) {
              //Sherlock Holmes
              vdp->last_height=480; 
              vdp->display_width=pce_display_width;
	      video_setsize((vdp->display_width), (vdp->last_height));
            }
...
pce_vdp pce_vdp_create(void)
{
	...
	if (retval->memory) {
	    ...
	    video_setsize(264*DEFAULT_SCALER_PCE, 256*DEFAULT_SCALER_PCE);
            pce_display_width=528;
	    video_setpal(64, pce_palbase_red, pce_palbase_green, pce_palbase_blue);

	    retval->last_width = 31;
	    retval->display_width = 528;
	    retval->last_height = 512;
	    retval->regs_l[11] = 31;
	} else {
	    free(retval);
	    retval = NULL;
    ...
    if (getenv("PCEFORCEH") != NULL) {
            int hei=atoi(getenv("PCEFORCEH"));
            video_setsize(256*DEFAULT_SCALER_PCE, hei*DEFAULT_SCALER_PCE);
            retval->last_height=hei*2;
            printf("FORCE CHANGE HEIGHT %d\n",hei);
